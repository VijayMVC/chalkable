//using System;
//using System.Collections.Generic;
//using System.Linq;
//using Chalkable.Data.School.DataAccess;
//using Chalkable.Data.School.Model;
//using Chalkable.Tests.Services.TestContext;
//using NUnit.Framework;

//namespace Chalkable.Tests.Services.School
//{
//    public class AttendanceServiceTest : BaseSchoolServiceTest
//    {
//        [Test]
//        public void TestSetGetAttendance()
//        {
//            var sy = SchoolYearServiceTest.CreateNextSchoolYear(FirstSchoolContext);
//            var mp = MarkingPeriodServiceTest.CreateNextMp(FirstSchoolContext, sy.Id, true);

//            FirstSchoolContext.AdminGradeSl.CalendarDateService.GetCalendarDateByDate(mp.StartDate);
//            var student = FirstSchoolContext.FirstStudent;
//            var course = FirstSchoolContext.AdminGradeSl.CourseService.Add("testCourse", "testCourse", null);
//            var c = FirstSchoolContext.AdminGradeSl.ClassService.Add(sy.Id, course.Id, "testClass", "testClass", FirstSchoolContext.FirstTeacher.Id,
//                                                                    student.StudentInfo.GradeLevelRef, new List<Guid> { mp.Id });

//            FirstSchoolContext.AdminGradeSl.ClassService.AddStudent(c.Id, student.Id);
//            FirstSchoolContext.AdminGradeSl.ClassService.AddStudent(c.Id, FirstSchoolContext.SecondStudent.Id);

//            var room = FirstSchoolContext.AdminGradeSl.RoomService.AddRoom("testRoomNumber", "testDesc", null, null, "testPhone");
//            var room2 = FirstSchoolContext.AdminGradeSl.RoomService.AddRoom("testRoomNUmer2", "testDesc2", null, null, "testPhone2");
//            var dates = FirstSchoolContext.AdminGradeSl.CalendarDateService.GetDays(mp.Id, false);
//            var schoolDates = dates.Where(x => x.IsSchoolDay).ToList();
//            var schoolDate = schoolDates[0];
//            var periods = FirstSchoolContext.AdminGradeSl.PeriodService.GetPeriods(mp.Id, schoolDate.ScheduleSectionRef);
//            var currentPeriod = periods[0];

//            var clPeriod =  FirstSchoolContext.AdminGradeSl.ClassPeriodService.Add(currentPeriod.Id, c.Id, room.Id);
//            var clPerson = FirstSchoolContext.AdminGradeSl.ClassService.GetClassPerson(c.Id, student.Id);
            
//            AssertForDeny(sl => sl.AttendanceService.SetClassAttendance(clPerson.Id, clPeriod.Id, schoolDate.DateTime, AttendanceTypeEnum.Absent)
//                , FirstSchoolContext, SchoolContextRoles.AdminViewer | SchoolContextRoles.FirstStudent | SchoolContextRoles.FirstParent
//                | SchoolContextRoles.SecondTeacher | SchoolContextRoles.Checkin);


//            var c2 = FirstSchoolContext.AdminGradeSl.ClassService.Add(sy.Id, course.Id, "testClass2", "testClass2"
//                     , FirstSchoolContext.FirstTeacher.Id, student.StudentInfo.GradeLevelRef, new List<Guid>{mp.Id});
//            var clPeriod2 = FirstSchoolContext.AdminGradeSl.ClassPeriodService.Add(currentPeriod.Id, c2.Id, room2.Id);
   
//            // same class check 

//            var attService = FirstSchoolContext.AdminGradeSl.AttendanceService;
//            AssertException<Exception>(() => attService.SetClassAttendance(clPerson.Id, clPeriod2.Id, schoolDate.DateTime, AttendanceTypeEnum.Absent));

//            var reason = FirstSchoolContext.AdminGradeSl.AttendanceReasonService.Add(AttendanceTypeEnum.Excused, "reason1", "reason1");
//            AssertException<Exception>(() => attService.SetClassAttendance(clPerson.Id, clPeriod.Id, schoolDate.DateTime, AttendanceTypeEnum.Absent, reason.Id));
            
//            var date2 = dates.First(x => x.ScheduleSectionRef.HasValue && x.ScheduleSectionRef != schoolDate.ScheduleSectionRef);
//            AssertException<Exception>(() => attService.SetClassAttendance(clPerson.Id, clPeriod.Id, date2.DateTime, AttendanceTypeEnum.Absent));

//            var attendace = attService.SetClassAttendance(clPerson.Id, clPeriod.Id, schoolDate.DateTime, AttendanceTypeEnum.Excused, reason.Id);
//            Assert.AreEqual(attendace.ClassPeriodRef, clPeriod.Id);
//            Assert.AreEqual(attendace.ClassPersonRef, clPerson.Id);
//            Assert.AreEqual(attendace.Date, schoolDate.DateTime);
//            Assert.AreEqual(attendace.AttendanceReasonRef, reason.Id);
//            var attendances = attService.GetClassAttendanceDetails(new ClassAttendanceQuery {Id = attendace.Id}, null);
//            AssertAreEqual(attendace, attendances[0]);

//            AssertForDeny(sl=> sl.AttendanceService.SetAttendanceForClass(clPeriod.Id, schoolDate.DateTime, AttendanceTypeEnum.Present), FirstSchoolContext
//                , SchoolContextRoles.FirstParent | SchoolContextRoles.SecondTeacher 
//                | SchoolContextRoles.FirstStudent | SchoolContextRoles.AdminViewer | SchoolContextRoles.Checkin);

//            AssertException<Exception>(() => attService.SetAttendanceForClass(clPerson.Id, date2.DateTime, AttendanceTypeEnum.Absent));
            
//            attService.SetAttendanceForClass(clPeriod.Id, schoolDate.DateTime, AttendanceTypeEnum.Present);

//            Assert.AreEqual(attService.GetClassAttendanceDetails(new ClassAttendanceQuery { Type = AttendanceTypeEnum.Late}, null).Count, 0);
//            Assert.AreEqual(attService.GetClassAttendanceDetails(null, null, null, null, null, schoolDate.DateTime.AddDays(-1)).Count, 0);
//            Assert.AreEqual(attService.GetClassAttendanceDetails(null, null, c2.Id, null, null, schoolDate.DateTime).Count, 0);
//            attendances = attService.GetClassAttendanceDetails(null, null, c.Id, null, null, schoolDate.DateTime);
//            Assert.AreEqual(attendances.Count, 2);
//            Assert.AreEqual(attService.GetClassAttendanceDetails(null, null, c.Id, student.Id, null, schoolDate.DateTime).Count, 1);
//            Assert.AreEqual(attendances[0].Type, AttendanceTypeEnum.Present);
//            Assert.AreEqual(attendances[1].Type, AttendanceTypeEnum.Present);

//        }

//        [Test]
//        public void TestGetSetDailAttendance()
//        {
//            var nowDate = FirstSchoolContext.AdminGradeSl.Context.NowSchoolTime;
//            var startSyDate = nowDate.Date.AddDays(-7);
//            var sy = SchoolYearServiceTest.CreateNextSchoolYear(FirstSchoolContext, startSyDate);
//            var mp = MarkingPeriodServiceTest.CreateNextMp(FirstSchoolContext, sy.Id);
//            FirstSchoolContext.AdminGradeSl.DayTypeService.GenerateDefaultSections(mp.Id);

//            FirstSchoolContext.AdminGradeSl.CalendarDateService.GetCalendarDateByDate(mp.StartDate);
//            var toDayDate = FirstSchoolContext.AdminGradeSl.CalendarDateService.GetCalendarDateByDate(nowDate.Date);
//            var currentTime = (int)(nowDate - nowDate.Date).TotalMinutes;
//            var currentPeriod = FirstSchoolContext.AdminGradeSl.PeriodService.Add(mp.Id, currentTime - 10, currentTime + 30, toDayDate.ScheduleSectionRef.Value, 0);

//            var student = FirstSchoolContext.FirstStudent;
//            var course = FirstSchoolContext.AdminGradeSl.CourseService.Add("testCourse", "testCourse", null);
//            var c = FirstSchoolContext.AdminGradeSl.ClassService.Add(sy.Id, course.Id, "testClass", "testClass", FirstSchoolContext.FirstTeacher.Id,
//                                                                    student.StudentInfo.GradeLevelRef, new List<Guid> { mp.Id });
//            FirstSchoolContext.AdminGradeSl.ClassService.AddStudent(c.Id, student.Id);
//            var room = FirstSchoolContext.AdminGradeSl.RoomService.AddRoom("testRoomNumber", "testDesc", null, null, "testPhone");
           

            
//            var clPeriod =  FirstSchoolContext.AdminGradeSl.ClassPeriodService.Add(currentPeriod.Id, c.Id, room.Id);
//            var clPerson = FirstSchoolContext.AdminGradeSl.ClassService.GetClassPerson(c.Id, student.Id);


//            AssertForDeny(sl=>sl.AttendanceService.GetDailyAttendance(toDayDate.DateTime, student.Id), FirstSchoolContext
//                , SchoolContextRoles.FirstParent | SchoolContextRoles.SecondStudent);
//            Assert.IsNull(FirstSchoolContext.AdminGradeSl.AttendanceService.GetDailyAttendance(toDayDate.DateTime, student.Id));
            
//            AssertForDeny(sl=>sl.AttendanceService.SetDailyAttendance(toDayDate.DateTime, student.Id,  currentPeriod.StartTime + 10, null)
//                , FirstSchoolContext, SchoolContextRoles.FirstStudent | SchoolContextRoles.FirstTeacher | SchoolContextRoles.FirstParent);

//            var attService = FirstSchoolContext.AdminGradeSl.AttendanceService;
//            var dailyAtt = attService.SetDailyAttendance(toDayDate.DateTime, student.Id, currentPeriod.StartTime + 10, null);
//            Assert.AreEqual(dailyAtt.Date, toDayDate.DateTime);
//            Assert.AreEqual(dailyAtt.PersonRef, student.Id);
//            Assert.AreEqual(dailyAtt.TimeIn, currentPeriod.StartTime + 10);
//            Assert.IsNull(dailyAtt.Arrival);
//            Assert.IsNull(dailyAtt.TimeOut);

//            var dailyAtt2 = attService.SetDailyAttendance(toDayDate.DateTime, student.Id, currentPeriod.StartTime, currentPeriod.EndTime);
//            AssertAreEqual(dailyAtt, dailyAtt2);
//            dailyAtt2 = attService.GetDailyAttendance(toDayDate.DateTime, student.Id);
//            AssertAreEqual(dailyAtt, dailyAtt2);

//            var att = attService.SetClassAttendance(clPerson.Id, clPeriod.Id, toDayDate.DateTime, AttendanceTypeEnum.Late);
//            dailyAtt2 = attService.GetDailyAttendance(toDayDate.DateTime, student.Id);
//            Assert.AreEqual(dailyAtt2.Arrival, (int)(att.LastModified - att.LastModified.Date).TotalMinutes);

//            FirstSchoolContext.AdminGradeSl.ClassService.AddStudent(c.Id, FirstSchoolContext.SecondStudent.Id);
//            var clPerson2 = FirstSchoolContext.AdminGradeSl.ClassService.GetClassPerson(c.Id, FirstSchoolContext.SecondStudent.Id);
//            var att2 = attService.SetClassAttendance(clPerson2.Id, clPeriod.Id, toDayDate.DateTime, AttendanceTypeEnum.Present);
//            dailyAtt2 = attService.GetDailyAttendance(toDayDate.DateTime, FirstSchoolContext.SecondStudent.Id);
//            Assert.AreEqual(dailyAtt2.Date, toDayDate.DateTime);
//            Assert.AreEqual(dailyAtt2.Arrival, currentPeriod.StartTime);
//            Assert.AreEqual(dailyAtt2.TimeIn, currentPeriod.StartTime);

//        }

//        [Test]
//        public void TestSwipeCard()
//        {
            
//        }

//        [Test]
//        public void NotAssignedAttendanceProcessTest()
//        {
//            var adminLocator = FirstSchoolContext.AdminGradeSl;
//            var nowDate = adminLocator.Context.NowSchoolTime;
//            var startSyDate = nowDate.Date.AddDays(-7);
//            var mp = MarkingPeriodServiceTest.CreateSchoolYearWithMp(FirstSchoolContext, startSyDate, true);
//            var time = (nowDate.Date - nowDate).Minutes;
//            var cDate = adminLocator.CalendarDateService.GetCalendarDateByDate(nowDate.Date);
//            var period = adminLocator.PeriodService.Add(mp.Id, time - 25, time + 25, cDate.ScheduleSectionRef.Value, 1);
//            var c = ClassServiceTest.CreateClass(FirstSchoolContext, FirstSchoolContext.FirstTeacher
//                                                 , FirstSchoolContext.FirstStudent, FirstSchoolContext.SecondStudent
//                                                 , "math", mp.SchoolYearRef);
//            var room = adminLocator.RoomService.AddRoom("1", "first room ", "45x45", null, "000000");
//            var classPeriod = adminLocator.ClassPeriodService.Add(period.Id, c.Id, room.Id);
            
//            AssertForDeny(sl=>sl.AttendanceService.NotAssignedAttendanceProcess(), FirstSchoolContext, SchoolContextRoles.AdminEditor
//                | SchoolContextRoles.AdminViewer | SchoolContextRoles.FirstTeacher | SchoolContextRoles.SecondTeacher
//                | SchoolContextRoles.FirstStudent | SchoolContextRoles.SecondStudent | SchoolContextRoles.Developer
//                | SchoolContextRoles.FirstParent);

//            adminLocator.AttendanceService.NotAssignedAttendanceProcess();
//            var notifications = FirstSchoolContext.FirstTeacherSl.NotificationService.GetNotifications(0, int.MaxValue);
//            Assert.AreEqual(notifications.Count, 1);
//            Assert.AreEqual(notifications[0].Type, NotificationType.NoTakeAttendance);
//            Assert.AreEqual(notifications[0].ClassPeriodRef, classPeriod.Id);
//            Assert.AreEqual(notifications[0].PersonRef, FirstSchoolContext.FirstTeacher.Id);
            
//        }
//    }
//}
