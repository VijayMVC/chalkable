...
namespace Chalkable.BusinessLogic.Services.School.Announcements
{
    ...
    public class AdminAnnouncementService : BaseAnnouncementService, IAdminAnnouncementService
    {
        ...
        {
            return DoRead(u => CreateDataAccess(u).GetLastFieldValues(personId, int.MaxValue));
        }
        ...
        {
            Trace.Assert(Context.PersonId.HasValue);
            return DoRead(u => CreateDataAccess(u).Exists(title, Context.PersonId.Value, excludedLessonPlaId));
        }
