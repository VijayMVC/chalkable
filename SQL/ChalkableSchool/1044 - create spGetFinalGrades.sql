...
select FinalGrade.Id as FinalGrade_Id,
	   ...
	   FinalGrade.Discipline as FinalGrader_Discipline,
...
select  
	 ...
	 StudentFinalGrade.GradeByParticipation as StudentFinalGrade,
	 vwPerson.*
...
begin
	...
	declare @fgAnnType table(PercentValue int, AnnouncementTypeId int, GradingStyle int)
	...
	begin
		...
		insert into @fgAnnType(PercentValue, AnnouncementTypeId, GradingStyle)
		select fgat.PercentValue, fgat.AnnouncementTypeRef, fgat.GradingStyle
	...
	begin
		insert into @fgAnnType(PercentValue, AnnouncementTypeId, GradingStyle)
		select @typePercent, at.id, @gradingStyle
		from @annTypes at
	end

	declare @finalGradeId uniqueidentifier = NEWID()
	...
	values(@finalGradeId, 0, @modePercent, @attendances, @desciplines, @gradingStyle, 0 , 0)
	...
	select NEWID(), @finalGradeId, fgAt.PercentValue, 
		   fgAt.AnnouncementTypeId, fgAt.GradingStyle, 0
    from @fgAnnType fgAt
