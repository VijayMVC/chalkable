...
left join Class on Class.Id = ClassPerson.ClassRef
left join MarkingPeriod on MarkingPeriod.Id = ClassPerson.markingPeriodRef and MarkingPeriod.SchoolYearRef = @schoolYearId
where
StudentSchoolYear.SchoolYearRef = @schoolYearId and Class.SchoolYearRef = @schoolYearId and
...
(@filter is null or FirstName like @filter or LastName like @filter) and
(@includeWithdraw = 1 or @includeWithdraw is null  or ClassPerson.IsEnrolled = 1)
group by
Student.Id,
Student.FirstName,
Student.LastName,
