Create Procedure spGetPossibleMessageRecipients 
	@callerId int,
	@callerRoleId int,
	@filter1 nvarchar(50), 
	@filter2 nvarchar(50),
...
Begin 
	...
	Begin
		 If @teacherClassOnly = 1
			Delete From @personT Where  not exists(
						Select *
						From ClassPerson
						Join ClassTeacher on ClassTeacher.ClassRef = ClassPerson.ClassRef
						Join Class c on c.Id = ClassTeacher.ClassRef
						Where ClassTeacher.PersonRef = @callerId and c.SchoolYearRef = @schoolYearId And ClassPerson.PersonRef = Id
					)  
	End
		...
		Begin
			...
			Begin
				Delete From @personT Where Not exists(
							...
							Where cp1.PersonRef = Id and c.SchoolYearRef = @schoolYearId And cp2.PersonRef = @callerId 
						)
			End
		End 
		If @teacherStudentMessagingEnabled = 1
		Begin
			If @teacherClassOnly = 1
				Delete From @personT Where not exists(
							...
							Where c.SchoolYearRef = @schoolYearId and ClassPerson.PersonRef = @callerId And ClassTeacher.PersonRef = Id
						) 
		End
		Else
