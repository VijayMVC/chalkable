...
from @cpIds cp
where ClassAttendance.ClassPersonRef = cp.id and ClassAttendance.ClassPeriodRef = @classPeriodId and ClassAttendance.Date = @date
...
where cp.id not in (select ClassPersonRef from ClassAttendance 
					where ClassAttendance.Id is not null and [Date] = @date and ClassPeriodRef = @classPeriodId)
...
select 	
	...
	ClassAttendance.Type as ClassAttendance_AttendanceType,
	ClassAttendance.AttendanceReasonRef as ClassAttendance_AttendanceReasonId,
...
begin 
	...
	where
		...
		and ((@type is null) 
			or (@a1 = 1 and ClassAttendance_AttendanceType = 1)
			or (@a2 = 1 and ClassAttendance_AttendanceType = 2)
			or (@a4 = 1 and ClassAttendance_AttendanceType = 4)
			or (@a8 = 1 and ClassAttendance_AttendanceType = 8)
			or (@a16 = 1 and ClassAttendance_AttendanceType = 16)
			)
end
else
begin
	...
	ClassPerson.Id as ClassPerson_Id,
	ClassPerson.PersonRef as ClassPerson_PersonRef,
	case when ClassAttendance.Id is null then 1 else ClassAttendance.Type end  as ClassAttendance_AttendanceType,
	ClassAttendance.AttendanceReasonRef as ClassAttendance_AttendanceReasonId,
