...
begin
	...
	begin
		...
		select StudentRef, 3 
		from StudentSchool
		where StudentRef = @callerId and SchoolRef = @schoolId and @hasStudentMyApps = 1
			and exists(select * from StudentSchoolYear where StudentRef = StudentSchool.StudentRef and SchoolYearRef = @schoolYearId)
	end
	if  @callerRoleId = 2
	begin
		insert into @localPersons (Id, RoleRef)
		select StudentRef, 3 
		from StudentSchool
		where (@canInstallForStudent = 1 and StudentRef in (select ClassPerson.PersonRef
															from ClassPerson
		 ...
		 select StaffRef, 2
		 from StaffSchool
		 where SchoolRef = @schoolId and StaffRef = @callerId and @canInstallForTeacher = 1
	end
	if @callerRoleId = 5 or @callerRoleId = 7 or @callerRoleId = 8
	begin
		insert into @localPersons (Id, RoleRef)
		select StudentRef, 3 from StudentSchool
		where @canInstallForStudent = 1 and SchoolRef = @schoolId
		union 
		select StaffRef, 2 from StaffSchool
