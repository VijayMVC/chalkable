...
Roster AS (
	SELECT
		PersonRef
	FROM
		ClassPerson cp JOIN StudentSchoolYear ssy 
			ON ssy.StudentRef = cp.PersonRef
	WHERE
		cp.ClassRef = @classId
...
ApplicationsCount AS (
	...
	FROM
		Applications a
		CROSS JOIN Roster r
		LEFT JOIN ApplicationInstall ai 
			ON ai.ApplicationRef = a.ApplicationRef AND ai.PersonRef = r.PersonRef AND ai.Active = 1 AND ai.SchoolYearRef= @SchoolYearId
...
FROM
	Applications a LEFT JOIN ApplicationsCount ac 
		ON ac.ApplicationRef = a.ApplicationRef