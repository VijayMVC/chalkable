...
class TestFeed(BaseAuthedTestCase):
    def test_feed(self):
        ...
        # marking items as 'All Items'
        self.settings_data_2 = {'option': '3'}
    ...
    def do_feed_list_and_verify(self, start, count=2500):
        ...
        list_items_json_unicode_2 = self.get_admin('/Feed/DistrictAdminFeed.json?' + 'gradeLevelIds=' + '&start=' + str(start) + '&complete=' + str(False) + '&count=' + str(count))
        dictionary_verify_annoucementviewdatas_all_2 = list_items_json_unicode_2['data']['annoucementviewdatas']
        ...
        for item in dictionary_verify_annoucementviewdatas_all_2:
            self.assertTrue((get_item_date(item) < self.current_date_minus_10 or (get_item_date(item) > self.current_date_plus_10),
