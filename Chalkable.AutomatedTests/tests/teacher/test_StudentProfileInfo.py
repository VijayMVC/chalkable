...
class TestStudentProfileInfo(BaseAuthedTestCase):
  ...
  def test_student_info_get(self):
    ...
    for t in ('active', 'address', 'age', 'birthdate', 'caneditlogin', 'displayname', 'email', 'firstname','fullname','gender','gradelevel', 'gradelevels','hasmedicalalert', 
        'healthconditions', 'id','isallowedinetaccess','lastname','login','parents','phones','role','salutation','schoolid','specialinstructions','spedstatus','studentcontacts'):
