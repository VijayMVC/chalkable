...
class TestMarkingUnreadAllMessages(BaseTestCase):
    def setUp(self):
    ...
    def internal_(self):
        if len(self.get_all_inbox_messages()) > 0:
            ...
            for i in self.get_all_inbox_messages():
                self.assertFalse(i['incomemessagedata']['read'], 'Message is not in the state False')
    ...
    def test_teacher_marking_unread_all_messages(self):
        self.internal_()
