...
class TestFeed(BaseAuthedTestCase):
    def test_feed(self):
        ...
        for key, value in self.var_messagingSettings_cut_off_string['data'].iteritems():
            if key == 'teachertostudentmessaginenabled':
                if value == True:
                    get_messages_inbox_all = self.get_foreign_student(
                    ...
                    if len(get_messages_inbox_all_data) > 0:
                        ...
                        post_delete = self.postJSON_foreign_student('/PrivateMessage/Delete.json?', data)
                    ...
                    get_messages_inbox_all = self.get_foreign_student('/PrivateMessage/List.json?' + 'start=' + str(0) + '&count=' + str(1000) + '&income=' + str(True) + '&role=' + '&classOnly=' + str(False))
                    get_messages_inbox_all_data = get_messages_inbox_all['data']
                    self.assertTrue(len(get_messages_inbox_all_data) == 3, 'student has 3 messages')

                else:
                    self.assertTrue(key == 'teachertostudentmessaginenabled' and value == False)
