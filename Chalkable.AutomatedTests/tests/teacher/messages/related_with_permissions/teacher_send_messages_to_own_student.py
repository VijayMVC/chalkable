...
class TestTeacherSendMessagesToOwnStudent(BaseTestCase):
    def setUp(self):
        ...
        update_messaging_settings = self.admin.get_json(
            ...
            '&studentToClassOnly=' + str(False) + '&teacherToStudentMessaging=' + str(
                True) + '&teacherToClassOnly=' + str(True))
    ...
    def internal_(self):
        ...
        get_messages_inbox_all = self.student.get_json(
        ...
        if len(get_messages_inbox_all_data) > 0:
            ...
            post_delete = self.student.post_json('/PrivateMessage/Delete.json?', data={"ids": sliced_list,
                    "income": True})
        ...
        for i in range(3):
            post_send = self.teacher.post_json('/PrivateMessage/Send.json', data={"body": "this is a body", "personId": 3688, "subject": "this is a subject"})  #student CATALINA AYERS

        # this piece of code isn't actual because I don't know an email of each user!
        get_messages_inbox_all = self.student.get_json('/PrivateMessage/List.json?' + 'start=' + str(0) + '&count=' + str(1000) + '&income=' + str(True) + '&role=' + '&classOnly=' + str(False))
        get_messages_inbox_all_data = get_messages_inbox_all['data']
        self.assertTrue(len(get_messages_inbox_all_data) == 3, 'student has 3 messages')

    def test_test_sending_message_to_own_student(self):
        self.internal_()
