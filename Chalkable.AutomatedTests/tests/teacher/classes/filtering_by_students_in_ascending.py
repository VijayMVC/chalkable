...
class TestFeed(BaseAuthedTestCase):
    def test_feed(self):
        ...
        if ('View Classroom' or 'View Classroom (Admin)' or 'Maintain Classroom' or 'Maintain Classroom (Admin)') in decoded_list_for_permissions:
            get_list_of_classes = self.get('/Class/ClassesStats.json?' + 'schoolYearId=' + str(school_year) + '&start=' + str(0) + '&count=' + str(100) + '&sortType=' + str(4) + '&teacherId=' + str(id_of_current_teacher))
            ...
            self.assertTrue(list_for_students == sorted(list_for_students), 'Students sorted in ascending order')
        else:
