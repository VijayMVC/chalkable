...
class TestFeed(BaseAuthedTestCase):
    def test_feed(self):
        ...
        if ('View Classroom' or 'View Classroom (Admin)' or 'Maintain Classroom' or 'Maintain Classroom (Admin)') in decoded_list_for_permissions:
            get_list_of_classes = self.get('/Class/ClassesStats.json?' + 'schoolYearId=' + str(school_year) + '&start=' + str(0) + '&count=' + str(100) + '&sortType=' + str(8) + '&teacherId=' + str(id_of_current_teacher) + '&filter=' + str("agri"))
            ...
            if len(get_list_of_classes['data']) > 0:
                ...
                for key in get_list_of_classes_data:
                    for key2, value2 in key.iteritems():
                        if key2 == 'teacherids':
                            self.assertTrue(id_of_current_teacher in value2, 'teacher is the owner of the class')
        else:
