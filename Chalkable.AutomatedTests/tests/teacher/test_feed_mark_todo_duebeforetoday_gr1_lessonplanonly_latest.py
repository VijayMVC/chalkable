# Feed > ToDo. ["Items due before today", "Gr 1", "Lesson Plan Only", "Earliest"]
...
#########################################################################################################################
        ...
        self.settings_data = {'sortType':'1', 'announcementType':'3', 'gradingPeriodId': self.gr_periods()[0]}
    ...
    def do_feed_list_and_verify(self, start, count=2500):
        ...
        if len(dictionary_verify_annoucementviewdatas_all) > 0:
            for item in dictionary_verify_annoucementviewdatas_all:
                ...
                self.assertLessEqual(decoded_list_1[0], enddate, 'Verify exprisesdate of activity ' + str(item["id"]))
                self.assertGreaterEqual(decoded_list_2[0], enddate, 'Verify exprisesdate of activity ' + str(item["id"]))
                list_for_date.append(enddate)
