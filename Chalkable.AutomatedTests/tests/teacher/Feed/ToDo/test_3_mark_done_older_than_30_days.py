...
class TestFeed(BaseAuthedTestCase):
    ...
    def do_feed_list_and_verify(self, start, count=2000):
        ...
        for item in dictionary_verify_annoucementviewdatas_all:
            ...
            if item['type'] == 3:
                #self.assertTrue((get_lesson_plan_start_date(item) >= self.current_date_minus_30) or (get_lesson_plan_start_date(item) < self.current_date_minus_30 and get_lesson_plan_end_date(item) >= self.current_date_minus_30),
                                  #   'Lesson plan is not marked as complete ' + str(item["id"]))
                self.assertTrue((get_lesson_plan_end_date(item) >= self.current_date_minus_30),
                                'Lesson plan is not marked as complete ' + str(item["id"]))
