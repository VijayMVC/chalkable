...
class TestFeed(BaseAuthedTestCase):
    def test_feed(self):
        ...
        # filter: gr. period 1, earliest
        self.settings_data = {'sortType': '0', 'gradingPeriodId': self.gr_periods()[0]}
    ...
    def do_feed_list_and_verify(self, start, count=2500):
        ...
        for item in dictionary_verify_annoucementviewdatas_all:
            ...
            if item['type'] == 3:
                self.assertTrue(
                    get_lesson_plan_start_date(item) >= decoded_list_1[0] and get_lesson_plan_end_date(item) <=
                    decoded_list_2[0],
                    'Lesson plan is out of the gr. period 1 ' + str(item["id"]))

            if item['type'] == 1 or item['type'] == 4:
                self.assertLessEqual(decoded_list_1[0], get_item_date(item),
                                     'Date of an activity/supplemental is out of the beginning of the gr. period 1 ' + str(
                                         item["id"]))

                self.assertGreaterEqual(decoded_list_2[0], get_item_date(item),
                                        'Date of an activity/supplemental is out of the end of the gr. period 1 ' + str(
                                            item["id"]))
