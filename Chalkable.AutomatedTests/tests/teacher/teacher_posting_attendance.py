        ...
        if 'View Classroom Attendance' in decoded_list:
            ...
            for class_id, mp_id in dict_for_clas_marking_period.items():
                for mp in mp_id:
                    ...
                    date_in_correct_format = datetime.strptime(str(random_date_for_attendance), '%Y-%m-%d').strftime('%m-%d-%Y')
                    get_class_list = self.get('/Attendance/ClassList.json?' + 'classId=' + str(class_id) + '&date=' + date_in_correct_format)
                    ...
                    if len(get_class_list['data'])>0:
                        for info_about_one_student in json_data_get_class_list:
                            list_for_students.append({"personid":info_about_one_student['personid'],"level":attendance_level,"attendancereasonid":attendance_reason_id})
                        dict = {"classId":class_id,"date":date_in_correct_format,"items":list_for_students}
                        post_attendance = self.postJSON('/Attendance/SetAttendance.json', dict)
                        get_class_list_second_time = self.get('/Attendance/ClassList.json?' + 'classId=' + str(class_id) + '&date=' + date_in_correct_format)
                        ...
                        for info_about_one_student_second_time in json_data_get_class_list_second_time:
                            date_in_cycle = datetime.strptime(str(info_about_one_student_second_time['date']), '%Y-%m-%d').strftime('%m-%d-%Y')
                            self.assertTrue(info_about_one_student_second_time['classid'] == class_id, 'class_id is not equal')
                            self.assertTrue(info_about_one_student_second_time['isposted'] == True, 'attendance is not posted ' + str(class_id))
