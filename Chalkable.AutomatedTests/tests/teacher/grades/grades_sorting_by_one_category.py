    ...
    def test_feed(self):
        ...
        for one_class in list_for_class_id:
            if one_class == 13861 or one_class == 13806 or one_class == 14011 or one_class == 14436:
                ...
                for i in class_summary_grids_data['classannouncementtypes']:
                    ...
                    if key == 'gradingperiods':
                        for i in value:
                            for key2, value2 in i.iteritems():
                                if key2 == 'id':
                                    if len(class_summary_grids_data['standards']) > 0:
                                        # sorting by one category
                                        self.get('/Grading/ClassGradingGrid?' + 'classId=' + str(
                                            one_class) + '&gradingPeriodId=' + str(
                                            value2) + '&standardId=' + '&classAnnouncementTypeId=' + str(one_random_activity_type) + '&notCalculateGrid=' + str(
                                            True))
