    ...
    def test_feed(self):
        ...
        for one_class in list_for_class_id:
            if one_class == 13861 or one_class == 13806 or one_class == 14011 or one_class == 14436:
                ...
                class_summary_grids = self.get('/Grading/FinalGrade?' + 'classId=' + str(one_class))
                class_summary_grids_data = class_summary_grids['data']

                if len(class_summary_grids_data['currentfinalgrade']['studentfinalgrades']) > 0:
                    for key, value in class_summary_grids.iteritems():
                        if key == 'gradingperiods':
                            for i in value:
                                for key2, value2 in i.iteritems():
                                    if key2 == 'id':
                                        data = {'classId': one_class,
                                                'gradingPeriodId': value2,
                                                }
                                        self.post('/Grading/PostGradebook', data)
