            ...
            VOID, function toggleCategories(node, event){
                ...
                if (selectedType == 'all'){
                    if (checkboxes.count() > 1){
                        this.dom.find('input[type=checkbox]:not(:disabled)')
                        ...
                        this.dom.find('input[name=app-category-all]').setAttr("disabled", true);
                    }
                }
                else{
                    this.dom.find('input[name=app-category-all]').trigger(eventName, [false]);
                    this.dom.find('input[name=app-category-all]').setAttr("disabled", false);
                    checkboxes.filter(function(el){
                        return el.is(":checked");
                    })
                    .forEach(function(el){
