...
namespace Chalkable.Web.Models.CalendarsViewData
{
    ...
    public class AttendanceForStudentCalendarItemViewData : AttendanceCalendarItemViewData
    {
        ...
        public static IList<AttendanceForStudentCalendarItemViewData> Create(IList<ClassAttendanceComplex> attendances)
        {
            return attendances.Select(x => Create(x, 0)).ToList();
        }
        public static AttendanceForStudentCalendarItemViewData Create(ClassAttendanceComplex attendances, int count)
        {
    ...
    public class AttendanceForStudentCalendarViewData : BaseAttendanceMonthCalendarViewData<AttendanceForStudentCalendarItemViewData>
    {
        ...
        public static AttendanceForStudentCalendarViewData Create(DateTime date, bool isCurrentMonth, Guid personId
            , IList<ClassAttendanceComplex> attendances)
        {
    ...
    public class AttendanceForClassCalendarItemViewData : BaseAttendanceMonthCalendarViewData<AttendanceCalendarItemViewData>
    {
        public Guid ClassId { get; set; }
        protected AttendanceForClassCalendarItemViewData(DateTime date, bool isCurrentMonth) : base(date, isCurrentMonth)
        {
        }

        public static AttendanceForClassCalendarItemViewData Create(DateTime date, bool isCurrentMonth, Guid classId, IList<ClassAttendanceComplex> attendances)
        {
            var groupedAtt = attendances.GroupBy(x => x.Type).ToDictionary(x => x.Key, x => x.Count());
            return new AttendanceForClassCalendarItemViewData(date, isCurrentMonth)
                {
                    ClassId = classId,
