...
Ext.ux.form.SuperBoxSelect = function(config) {
    ...
    this.addEvents(
        ...
        'clear',
        /**
         * Fires after the item value is changed.
         * @event changeditem
         * @memberOf Ext.ux.form.SuperBoxSelect
         * @param {SuperBoxSelect} this
         */
...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
    ...
    /**
     * @cfg {Boolean} withoutButtons Is there shold be the buttons.
     */
    ...
    onRender:function(ct, position) {
        ...
        this.el.on('keyup', function(e){
            ...
            if(len > 1){
                if(e.getKey() === e.RIGHT){
                    this.items.items[0].input.focus();
                }
                if(e.getKey() === e.LEFT){
                    this.items.items[len-2].input.focus();
                }
            }
        }.bind(this));
    ...
    addRecord : function(record) {
        ...
        this.addItemBox(val, display, caption, cls, style, store);
    ...
    addItem : function(newItemObject, /*hidden param*/ forcedAdd){
        ...
        if(this.mode === 'remote' && !this.localOnAdd){
    ...
    addItemBox : function(itemVal,itemDisplay,itemCaption, itemClass, itemStyle, store) {
...
Ext.ux.form.SuperBoxSelectItem = Ext.extend(Ext.ux.form.SuperBoxSelectItem,Ext.Component, {
    ...
    onRender : function(ct, position){
        ...
        this.index = this.owner.items.length;

        this.input = el.createChild({
            tag: 'input',
            'class': 'x-superboxselect-item-input',
            'index': this.index
        });
        ...
        this.input.on('keydown', function(e){
            var mainInput = jQuery(that.owner.inputEl.dom).find('#recipients-combo');
            var index = this.dom.attributes.index.value;
            if(e.getKey() === e.BACKSPACE){
                that.onLnkClick();
                setTimeout(function(){
                    var inputs = jQuery(that.owner.el.dom).parents('#x-form-el-recipients-combo').find('.x-superboxselect-item-input');
                    inputs.each(function(index, item){
                        jQuery(this).attr('index', index);
                    });
                    if(that.owner.items.items[index]){
                        that.owner.items.items[index].input.focus();
                    }else{
                        mainInput.focus();
                    }
                }, 300);
                ...
                var len = that.owner.items.length;
                if((e.getKey() === e.RIGHT && index < len - 1) || (e.getKey() === e.LEFT && index > 0 && len > 1)){
                    that.owner.items.items[e.getKey() === e.RIGHT ? parseInt(index, 10) + 1 : index - 1].input.focus();
                }else{
                    if(e.getKey() === e.RIGHT && index < len ){
                        that.owner.items.items[0].input.focus();
                    }else{
                        if(e.getKey() === e.LEFT && index == 0 ){
                            that.owner.items.items[len-1].input.focus();
                        }else{
                            if(e.getKey() !== e.UP && e.getKey() !== e.DOWN){
                                var val = String.fromCharCode(e.getCharCode())[e.shiftKey ? 'toUpperCase' : 'toLowerCase']();
                                setTimeout(function(){
                                    mainInput.setValue(val).focus();
                                    this.dom.value = '';
                                }.bind(this), 1);
        ...
        if(this.store){
            new Ext.form.ComboBox({
                ...
                renderTo: el,
                lazyRender:true,
                listClass: 'grey-list super-list',
                store: this.store,
                valueField: 'id',
                displayField: 'text',
                ctCls: 'new-combo',
                ...
                listeners: {
                    select: function(combo, record, index){
                        this.hidden.dom.value = this.value = combo.getValue();
                        this.owner.fireEvent('changeditem', combo, record, this.index, index);
                    },
