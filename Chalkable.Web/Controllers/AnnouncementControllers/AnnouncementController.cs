...
namespace Chalkable.Web.Controllers.AnnouncementControllers
{
    ...
    public class AnnouncementController : AnnouncementBaseController
    {
        ...
        [AuthorizationFilter("DistrictAdmin, Teacher")]
        public ActionResult Edit(int announcementId, int type)
        {
            var viewData = PrepareFullAnnouncementViewData(announcementId, (AnnouncementType)type);
        ...
        {
            var res = PrepareFullAnnouncementViewData(announcementId, (AnnouncementType?)announcementType, true);
        ...
        public ActionResult AddStandard(int announcementId, int standardId, int type)
        {
            SchoolLocator.GetAnnouncementService((AnnouncementType)type).AddAnnouncementStandard(announcementId, standardId);
            return Json(PrepareFullAnnouncementViewData(announcementId, (AnnouncementType)type));
        }

        [AuthorizationFilter("Teacher")]
        public ActionResult RemoveStandard(int announcementId, int standardId, int type)
        {
            SchoolLocator.GetAnnouncementService((AnnouncementType)type).RemoveStandard(announcementId, standardId);
            return Json(PrepareFullAnnouncementViewData(announcementId, (AnnouncementType)type));
        }
